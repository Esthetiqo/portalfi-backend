...
import { CreatePhysicalCardOrderDto, ConfirmCardOrderDto } from '../../common/dto/gnosispay-base.dto';
...
export class GnosisPayCardOrdersController {
  ...
  @ApiResponse({ status: 200, description: 'Card orders retrieved successfully' })
  ...
  @ApiResponse({ status: 201, description: 'Physical card order created successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Card order retrieved successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Card order cancelled successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Card order confirmed successfully' })
  ...
  @ApiParam({ name: 'orderId', description: 'Order ID for which to create physical card' })
  ...
  @ApiResponse({ status: 400, description: 'Order not confirmed or payment not completed' })
  ...
  @ApiResponse({
    ...
    schema: {
      properties: {
        ...
        newTotal: { type: 'string', description: 'New order total after discount' },
      },
    },
  })
  @ApiResponse({ status: 400, description: 'Invalid coupon code or order not eligible' })
  @ApiResponse({ status: 404, description: 'Order not found' })
  async attachCouponToOrder(
    ...
    return await this.httpService.attachCouponToOrder(token, orderId, body.couponCode);
  }

  @Post(':orderId/attach-transaction')
  @ApiParam({ name: 'orderId', description: 'Order ID to register payment for' })
  @ApiOperation({ summary: 'Register payment transaction hash for physical card order' })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 400, description: 'Invalid transaction hash or order already paid' })
  @ApiResponse({ status: 404, description: 'Order not found' })
  async attachTransactionToOrder(
    ...
    return await this.httpService.attachTransactionToOrder(token, orderId, body.transactionHash);
  }
}
